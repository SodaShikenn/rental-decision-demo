...
| Commute | Select a Tokyo hub or confirm a searched destination; default to 08:00 arrival / 18:00 return departure in Japan time. Retrieve both directions independently; compare returned alternatives and confirm intent. | Google officially excludes Japan transit from Routes API. Live six-direction check returned no routes; walking worked. Maps links are available; in-app Japanese public-transport times need another provider. |
...
- **2026-09-22:** `npm test` — 108 frontend + 130 backend tests passed; 2 real-OCR tests skipped. Covers calculations, uncertainty, matching, automatic research, stale-response rejection, proposals, provider errors and storage boundaries.
