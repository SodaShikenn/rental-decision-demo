...
- 賃料、住所、最寄駅、間取り、面積、竣工年のサンプル抽出結果を確認・修正
- 未取得の経路・周辺情報を明示したうえで、画像の物件を暫定候補へ追加
...
現在の画像解析は、募集図面形式を想定した固定サンプル抽出です。アップロード画像は外部送信しません。実運用では画像をサーバー側のVision/OCR APIへ送り、抽出値、信頼度、画像内の根拠領域を返す構成を想定しています。チャットは評価根拠を確認しやすくするためのルールベース実装です。物件、経路、掲載賃料、レビュー、施設評価はすべて架空のデモデータです。
...
|募集図面の構造化|サーバー側Vision/OCR API|画像を一時処理し、フィールド単位の信頼度と根拠領域を返して人が確定する|
