...
class LeisureRequest(BaseModel):
    ...
    categories: list[Literal['park', 'gym', 'cafe']] = Field(default_factory=lambda: ['park', 'gym', 'cafe'], min_length=1, max_length=3)
    destinationId: str | None = Field(default=None, pattern=r'^[A-Za-z0-9_-]{5,256}$')

    @model_validator(mode='after')
    def unique(self):
        if len({c.id for c in self.candidates}) != len(self.candidates) or len(set(self.categories)) != len(self.categories):
