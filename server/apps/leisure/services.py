...
async def discover_leisure(body, key, *, transport=None):
    async with httpx.AsyncClient(timeout=20, transport=transport) as client:
        ...
        if body.destinationId:
            destination = await google.details(
                body.destinationId,
        ...
        async def walk(origin, place):
            try:
                ...
                routes = [
                    ...
                    if (v := route_view(r, "WALK")) is not None
                ]
                            ...
                            "places": [
                                {
                                    **place_view(p),
                                    "route": r,
                                    "hours": p.get("regularOpeningHours", {}).get(
                                        "weekdayDescriptions", []
                                    ),
                                    "routeUrl": directions_url(
                                        c.address,
                ...
                return {
                    **base,
                    ...
                    "groups": await asyncio.gather(
                        *[
                            group(k)
                            for k in (["regular"] if destination else body.categories)
                        ]
                    ),
                }
            except AppError:
        ...
        return {
            ...
            "radiusMeters": None if destination else 1500,
