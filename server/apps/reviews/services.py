...
from providers.google_maps import GoogleMaps, place_view, distance

DETAIL_FIELDS = (
    "id,displayName,formattedAddress,location,googleMapsUri,attributions,types"
)
...
def matches_building(place, name, origin):
    ...
    return bool(
        len(wanted) >= 3
        and found == wanted
        and not set(place.get("types", []))
        & {"cafe", "restaurant", "gym", "store", "lodging", "supermarket"}
        and place.get("location")
        and distance(origin["location"], place["location"]) <= 250
    )
...
async def search_reviews(body, key, *, transport=None):
    async with httpx.AsyncClient(timeout=20, transport=transport) as client:
        ...
        matched = [
            place_view(p) for p in places if matches_building(p, body.name, origin)
        ]
        return {
            "status": "choose_place" if matched else "entity_unverified",
            "places": matched,
...
async def read_reviews(body, key, *, transport=None):
    async with httpx.AsyncClient(timeout=20, transport=transport) as client:
        ...
        if not origin or not matches_building(place, body.name, origin):
            raise AppError(
                409,
                "review_entity_unverified",
                "候補の建物と地点の一致を確認できません。再検索してください。",
            )
        details = await google.details(
            body.placeId, "id,reviews,rating,userRatingCount"
        )
        reviews = []
            ...
            reviews.append(
                {
                    ...
                    "originalText": r.get("originalText", {}).get("text", ""),
                    "rating": r.get("rating"),
                    ...
                    "url": r["googleMapsUri"],
                    "reportUrl": r.get("flagContentUri", ""),
                }
            )
        return {
            "place": place_view(place),
            "reviews": reviews,
            "rating": details.get("rating"),
            "count": details.get("userRatingCount"),
            "scope": "building_place",
            "order": "provider_relevance",
            "checkedAt": datetime.now(timezone.utc).isoformat(),
        }
