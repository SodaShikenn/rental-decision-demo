...
router = APIRouter(prefix='/api/shares', tags=['sharing'], dependencies=[Depends(apply_rate_limit)])


def store_for(request):
    settings = request.app.state.settings
    if not settings.sharing_enabled:
        raise AppError(503, 'sharing_disabled', 'リンク共有は無効です。ファイル出力を使えます。')
    return ShareStore(settings.share_db_path)


@router.post('')
def create(body: ShareCreate, request: Request):
    return json_response(store_for(request).create(body.brief.model_dump(mode='json'), body.expiresInDays), 201)
