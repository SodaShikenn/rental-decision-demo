...
class ShareStore:
    ...
    def connect(self):
        ...
        db.execute('CREATE TABLE IF NOT EXISTS shares (id TEXT PRIMARY KEY, secret TEXT NOT NULL, body TEXT NOT NULL, expires REAL NOT NULL)')
    ...
    def create(self, brief, days):
        ...
        with self.connect() as db:
            ...
            if db.execute('SELECT COUNT(*) FROM shares').fetchone()[0] >= self.capacity:
                raise AppError(503, 'share_capacity', '共有の保存容量に達しました。ファイル出力を使ってください。')
            db.execute('INSERT INTO shares VALUES (?, ?, ?, ?)', (digest(token), digest(secret), json.dumps(brief, ensure_ascii=False), expires))
        return {'token': token, 'deleteSecret': secret, 'expiresAt': expires}

    def read(self, token):
        with self.connect() as db:
            db.execute('DELETE FROM shares WHERE expires <= ?', (self.clock(),))
            row = db.execute('SELECT body, expires FROM shares WHERE id = ?', (digest(token),)).fetchone()
        if not row:
            raise AppError(404, 'share_not_found', '共有が見つからないか、期限切れ・削除済みです。')
        return {'brief': json.loads(row[0]), 'expiresAt': row[1]}

    def revoke(self, token, secret):
        with self.connect() as db:
            row = db.execute('SELECT secret FROM shares WHERE id = ? AND expires > ?', (digest(token), self.clock())).fetchone()
            if not row or not hmac.compare_digest(row[0], digest(secret)):
                raise AppError(404, 'share_not_found', '削除できる共有が見つかりません。')
