...
def route_view(route, mode):
    ...
    transit = [
        s.get("transitDetails") for s in steps if s.get("travelMode") == "TRANSIT"
    ]
    walking = [
        minutes(s.get("staticDuration")) for s in steps if s.get("travelMode") == "WALK"
    ]
    complete_steps = bool(steps) and all(s.get("travelMode") for s in steps)
    walk = (
        sum(walking) if complete_steps and all(m is not None for m in walking) else None
    )
    lines = []
    for detail in transit:
        ...
        lines.append(
            {
                "name": line.get("nameShort") or line.get("name", "路線名未取得"),
