...
def test_nearby_no_results_and_failed_routes_remain_distinct():
    ...
    body = LeisureRequest(
        candidates=[{"id": "a", "name": "A", "address": "東京都渋谷区一丁目"}],
        categories=["park", "gym"],
    )
    result = asyncio.run(
        discover_leisure(body, "key", transport=httpx.MockTransport(handler))
    )
...
def test_ambiguous_origin_does_not_search_nearby():
    ...
    body = LeisureRequest(
        candidates=[{"id": "a", "name": "A", "address": "東京都渋谷区"}]
    )
    result = asyncio.run(
        discover_leisure(body, "key", transport=httpx.MockTransport(handler))
    )
