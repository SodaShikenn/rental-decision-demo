...
import { ModelOutputSchema, toContract, normaliseBox, FIELD_KEYS } from "../../../src/apps/listing/models.js";
...
test("mock output satisfies the model schema", () => {
  ...
  assert.doesNotThrow(() => format.parse(JSON.stringify(mockReading(image))));
});

test("toContract returns every field with normalised evidence", () => {
  const contract = toContract(mockReading(image), meta);
...
test("a missing value is unknown: confidence becomes null", () => {
  const output = mockReading(image);
