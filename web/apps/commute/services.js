...
export function selectedRoutes(result) {
  return (result?.candidates || []).flatMap((c) =>
    c.status === "checked" && c.recommended != null
      ? [{ id: c.id, name: c.name, ...c.routes[c.recommended] }]
      : [],
  );
}
export function commuteQuestion(result) {
  ...
  return {
    evidence: routes.map((r) => `${r.name}：片道${r.minutes}分`).join("／"),
    text: `片道${Math.min(...values)}${new Set(values).size > 1 ? `〜${Math.max(...values)}` : ""}分。この通勤で何を優先したいですか？`,
  };
}
export function commutePreference(result, objective, level) {
  return {
    text: `通勤は${OBJECTIVES[objective]}ことを${level === "must" ? "重視" : "できれば希望"}。経路・時刻はその都度再確認する。`,
    level,
    source: "commute",
    details: { objective, daysPerWeek: result.schedule.daysPerWeek },
  };
}
