...
import { sourceLink, attribution, mapsCredit, formatTime } from '../../shared/maps.js';
...
function fareText(fare) {
  ...
  return Number.isFinite(value)?`${value.toLocaleString('ja-JP')} ${e(fare.currencyCode)}（片道目安）`:'運賃未取得';
}
export function resultsMarkup(result) {
  ...
  return `<p>${mapsCredit} · ${e(formatTime(result.checkedAt))}取得 · ${sourceLink(result.destination.url,result.destination.name)}</p><p class="section-hint">${e(result.destination.address)} ${attribution(result.destination.attributions)}</p>
    ...
    ${question?`<div class="context-question"><p>${e(question.evidence)}</p><h4>${e(question.text)}</h4><div class="choice-row">${Object.entries(OBJECTIVES).map(([key,label])=>`<button class="button" data-commute-choice="${key}">${label}</button>`).join('')}<button class="button button--text" data-commute-choice="later">まだ決めない</button></div><div id="commuteConfirm"></div></div>`:''}`;
}
