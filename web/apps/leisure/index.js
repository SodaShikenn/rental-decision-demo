...
export function initApp(app){
  ...
  $('#leisureConfirm').addEventListener('submit',event=>{event.preventDefault();if(!CATEGORIES[selected]||!result)return;const f=Object.fromEntries(new FormData(event.target));const note=leisurePreference(selected,f.frequency,f.level);saveNote(store,note.text,note.level,note.source,note.details);$('#leisureConfirm').textContent='希望と条件メモに反映しました。';});
