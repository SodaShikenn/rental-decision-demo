...
export function initApp(app) {
  ...
  const render = () => {
    ...
    $("#discoveredPriorities").innerHTML = confirmed.length ? confirmed.map(({ key, label }) => `<li>${label}：${formatPriority(key, priorities[key].value)}${key === "area" ? "以上" : "以下"} · ${LEVELS[priorities[key].level]} <button class="link-button" data-reconsider="${key}" type="button">考え直す</button></li>`).join("") : "<li>まだ希望は決めていません。次の質問から一緒に絞りましょう。</li>";
    ...
    $("#discoveryProgress").textContent = `${Math.min(progress, questions.length)} / ${questions.length} の視点を確認`;
