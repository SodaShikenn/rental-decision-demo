...
export function initApp(app) {
  ...
  function render() {
    ...
    $("#scenarioResults").innerHTML =
      `<p class="section-hint">週${days}日の試算。${commute ? "経路は取得済みの特定日時の推定を使用。" : "通勤時間は Google Maps で確認できます。地図の検索結果は自動取得されません。"}往復合計は両方向の取得時だけ表示します。遅延・定期券は推測しません。</p>${rows.map((r) => `<article class="journey-candidate"><h4>${e(r.name)}</h4><p>月額：${r.monthly == null ? "未確認（参考価格は含めません）" : yen(r.monthly)}</p><p>通勤：${r.route ? `${r.route.minutes}分／片道 · 徒歩${r.route.walkingMinutes ?? "未取得"}分 · 乗換${r.route.transfers ?? "未取得"}回` : "未取得"}<br>週の往路合計：${r.weeklyOutboundMinutes == null ? "未取得" : `${r.weeklyOutboundMinutes}分`}<br>週の往復合計：${r.weeklyRoundTripMinutes == null ? "未取得" : `${r.weeklyRoundTripMinutes}分`}</p><p class="section-hint">経路の比較軸：${OBJECTIVES[r.objective]}（${r.objectiveConfirmed ? "確認した希望" : "今回の試算条件・希望未確認"}）</p>${r.leisure.map((i) => `<p>希望した${CATEGORIES[i.kind] || e(i.kind)}：${i.minutes == null ? "徒歩・営業状態の確認が必要" : `取得した営業施設への徒歩 ${i.minutes}分`}</p>`).join("")}<ul>${r.advantages.map((a) => `<li>${e(a)}</li>`).join("")}${r.gaps.map((g) => `<li>次の確認：${e(g)}</li>`).join("")}</ul><p class="section-hint">${days === 0 ? "通勤なしの想定です。出社が増えれば比較が変わります。" : "頻度や経路、未確認の費用が変わると、この説明も変わります。"}</p></article>`).join("") || "<p>候補を追加して比較できます。</p>"}`;
  }
