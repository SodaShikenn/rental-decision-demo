// Image checks that run before any bytes leave the Worker.
// Limits follow the Claude vision docs for high-resolution-tier models
// (Claude 4.7 and later, including claude-opus-5).
...
export function fitsWithoutResize(width, height, maxEdge = MAX_EDGE, maxTokens = MAX_VISUAL_TOKENS) {
  return (
...
/**
 * Validate an uploaded image. Returns { ok: true, image } or { ok: false, status, code, message }.
...
export function validateImage(bytes) {
  if (bytes.length === 0) return { ok: false, status: 400, code: "empty_image", message: "画像ファイルが空です。" };
  if (bytes.length > MAX_BYTES) return { ok: false, status: 413, code: "image_too_large", message: "画像は5MB以下にしてください。" };
  const image = sniffImage(bytes);
  if (!image) return { ok: false, status: 415, code: "unsupported_image", message: "PNG / JPEG / WEBP 形式の画像を選択してください。" };
  if (!image.width || !image.height) return { ok: false, status: 415, code: "unreadable_image", message: "画像サイズを読み取れませんでした。" };
  if (!fitsWithoutResize(image.width, image.height)) {
    return {
      ok: false,
      status: 422,
      code: "image_needs_resize",
      message: `画像サイズ ${image.width}×${image.height}px が解析上限を超えています。縮小してから送信してください。`,
    };
  }
  return { ok: true, image: { ...image, bytes } };
}
