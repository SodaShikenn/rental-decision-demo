    ...
    beta: {
      messages: {
        create: async (params) => {
          ...
          return { model: MODEL, usage: { input_tokens: 3200, output_tokens: 900 }, stop_reason: "end_turn", ...reply };
        },
      },
    },
  };
...
test("sends the image with fallbacks, structured output, and no silent resizing", async () => {
  ...
  const result = await extractListing(image, {}, client);
...
test("effort comes from EXTRACTION_EFFORT, falling back to high for unknown values", async () => {
  ...
  await extractListing(image, { EXTRACTION_EFFORT: "medium" }, medium);
  assert.equal(medium.calls[0].output_config.effort, "medium");
  const unknown = stubClient({ content: [{ type: "text", text: goodText }] });
  await extractListing(image, { EXTRACTION_EFFORT: "turbo" }, unknown);
...
test("after a server-side fallback, the last text block is the answer", async () => {
  ...
  const { output } = await extractListing(image, {}, client);
  assert.equal(output.fields.layout.value, "1K");
});

const rejectsWith = (promise, status, code) =>
  assert.rejects(promise, (error) => error instanceof ExtractionError && error.status === status && error.code === code);

test("a refusal is reported before any output is parsed", async () => {
  await rejectsWith(extractListing(image, {}, stubClient({ stop_reason: "refusal", content: [] })), 502, "refused");
});

test("truncated output is reported as incomplete", async () => {
  await rejectsWith(extractListing(image, {}, stubClient({ stop_reason: "max_tokens", content: [{ type: "text", text: "{" }] })), 502, "incomplete");
});

test("output that does not match the schema is rejected", async () => {
  await rejectsWith(extractListing(image, {}, stubClient({ content: [{ type: "text", text: '{"fields":{}}' }] })), 502, "invalid_output");
  await rejectsWith(extractListing(image, {}, stubClient({ content: [] })), 502, "invalid_output");
});

test("SDK errors map to client-safe statuses", async () => {
  await rejectsWith(extractListing(image, {}, stubClient(new Anthropic.RateLimitError(429, {}, "rate", new Headers()))), 503, "upstream_busy");
  await rejectsWith(extractListing(image, {}, stubClient(new Anthropic.APIConnectionTimeoutError())), 504, "upstream_timeout");
  await rejectsWith(extractListing(image, {}, stubClient(new Anthropic.AuthenticationError(401, {}, "auth", new Headers()))), 503, "not_configured");
  await rejectsWith(extractListing(image, {}, stubClient(new Anthropic.BadRequestError(400, {}, "bad", new Headers()))), 422, "image_rejected");
  await rejectsWith(extractListing(image, {}, stubClient(new Anthropic.InternalServerError(500, {}, "boom", new Headers()))), 502, "upstream_error");
});
