...
import { sniffImage, validateImage, fitsWithoutResize, MAX_BYTES } from "../src/image.js";
...
test("validateImage maps each failure to a status and code", () => {
  assert.equal(validateImage(new Uint8Array()).status, 400);
  assert.deepEqual(pick(validateImage(new Uint8Array(MAX_BYTES + 1))), { status: 413, code: "image_too_large" });
  assert.deepEqual(pick(validateImage(new TextEncoder().encode("hello"))), { status: 415, code: "unsupported_image" });
  assert.deepEqual(pick(validateImage(pngHeader(4032, 3024))), { status: 422, code: "image_needs_resize" });
  const ok = validateImage(fixture);
  assert.equal(ok.ok, true);
  assert.equal(ok.image.mediaType, "image/png");
  assert.equal(ok.image.bytes, fixture);
});

const pick = ({ status, code }) => ({ status, code });
